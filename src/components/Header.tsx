  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-navy/98 backdrop-blur-lg shadow-lg border-b border-white/10"
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  to={item.href}
                  className={`relative px-4 py-2 transition-colors text-sm font-medium group block ${
                ...
                >
                  ...
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-primary transition-all duration-300 ${
                    location.pathname === item.href ? "w-1/2" : "w-0 group-hover:w-1/2"
                  }`} />
                </Link>
              </motion.div>
            ))}
